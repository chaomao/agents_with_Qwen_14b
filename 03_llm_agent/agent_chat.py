...
def agent_chat(query: str = Body(..., description="用户输入", examples=["恼羞成怒"]),
               history: List[History] = Body([],
                                             description="历史对话",
                                             examples=[[
                                                 {"role": "user", "content": "我们来玩成语接龙，我说一个，你说一个，我先来，厚积薄发"},
...
if __name__ == '__main__':
    ...
    while True:
        ...
        print(f"千问: {output}")
        history.append(History(role='user', content=human_input))
        history.append(History(role='assistant', content=output))
